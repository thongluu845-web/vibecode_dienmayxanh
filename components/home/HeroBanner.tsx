...
export default function HeroBanner({ banners }: HeroBannerProps) {
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden shadow-xl" style={{ aspectRatio: "16/6" }}>
      {banners.map((banner, idx) => (
        <div key={banner.id}
          ...
          <Link href={banner.link} className="block w-full h-full">
            <Image src={banner.image} alt={banner.title} fill priority={idx === 0}
              sizes="(max-width:768px) 100vw,1200px" className="object-cover"
            ...
            <div className="absolute inset-0 flex items-center px-8 sm:px-12">
              <div className="max-w-sm text-white" key={`${animKey}-${idx}`}>
                <p className="text-xs sm:text-sm font-bold text-yellow-400 uppercase tracking-widest mb-2"
                  style={{ animation: idx === cur ? "fadeInLeft 0.5s 0.1s both" : "none" }}>
                  ⚡ Siêu ưu đãi hôm nay
                </p>
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black leading-tight mb-3 drop-shadow-lg"
                  style={{ fontFamily: "'Rubik', sans-serif", animation: idx === cur ? "fadeInLeft 0.55s 0.2s both" : "none" }}>
                  {banner.title}
                </h2>
                <p className="text-sm sm:text-base text-gray-200 mb-5"
                  style={{ animation: idx === cur ? "fadeInLeft 0.55s 0.35s both" : "none" }}>
                  {banner.subtitle}
                </p>
                <span className="inline-flex items-center gap-2 bg-[#ff6600] hover:bg-[#e55a00] text-white
                  font-bold px-6 py-3 rounded-2xl text-sm transition-all duration-200
                  shadow-lg shadow-orange-500/40 hover:shadow-xl hover:-translate-y-0.5 active:scale-95 shine-hover"
      ...
      {/* Arrows */}
      {[
        { fn: prev, dir: "left",  Icon: ChevronLeftOutlined,  pos: "left-3"  },
        { fn: next, dir: "right", Icon: ChevronRightOutlined, pos: "right-3" },
      ].map(({ fn, dir, Icon, pos }) => (
        <button key={dir} onClick={fn}
          ...
          <Icon style={{ fontSize: 20 }} className="text-gray-700" />
        </button>
      ))}
      ...
      <div className="absolute bottom-3 right-4 z-20 text-xs text-white/70 font-medium">{cur + 1} / {banners.length}</div>
    </div>
  );
}
