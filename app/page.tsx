  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 sm:py-6 space-y-8 sm:space-y-12">
          ...
          <aside className="hidden lg:flex flex-col gap-1.5">
            {categories.slice(0, 8).map((cat, i) => (
              <a key={cat.id} href={`/danh-muc/${cat.slug}`}
                className="flex items-center gap-3 bg-white hover:bg-blue-50 hover:text-blue-600
                  px-3 py-2.5 rounded-xl shadow-sm hover:shadow-md
                  transition-all duration-200 group cursor-pointer hover:-translate-x-0.5"
                style={{ animationDelay: `${i * 50}ms` }}
              >
                <span className="text-xl w-7 text-center flex-shrink-0 group-hover:scale-125 transition-transform duration-200">
                  {cat.icon}
                </span>
                <span className="text-sm font-semibold text-gray-700 group-hover:text-blue-600 flex-1 min-w-0 truncate">
                  {cat.name}
                </span>
        ...
        <ScrollReveal variant="up" delay={100}>
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            <h2 className="text-xl sm:text-2xl font-black text-gray-900 flex items-center gap-2 mb-5"
              style={{ fontFamily: "'Rubik', sans-serif" }}>
              ...
              {brands.map((brand, i) => (
                <a key={brand} href={`/san-pham?brand=${brand.toLowerCase()}`}
                  className="flex-shrink-0 sm:flex-shrink bg-gray-50 border-2 border-transparent
                    ...
                    rounded-2xl px-4 py-3 flex items-center justify-center
                    text-sm font-black text-gray-500 hover:text-blue-600
