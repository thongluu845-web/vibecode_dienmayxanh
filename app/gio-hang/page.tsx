  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <h1 className="text-2xl font-black text-gray-900 mb-6 flex items-center gap-2">
        <ShoppingCartOutlined style={{ fontSize: 24 }} className="text-blue-600" />
        Giỏ hàng của bạn
      </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1 bg-white rounded-2xl shadow-sm p-12 text-center">
          <div className="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-5">
            <ShoppingCartOutlined style={{ fontSize: 40 }} className="text-blue-300" />
          </div>
        ...
        <div className="lg:w-80">
          <div className="bg-white rounded-2xl shadow-sm p-5 sticky top-24">
            ...
            <button disabled className="w-full bg-red-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-2xl mb-4">
              Đặt hàng ngay
            </button>
            <div className="space-y-2">
              {[
                { Icon: SecurityTwoTone,      text: "Thanh toán an toàn & bảo mật" },
                { Icon: LocalShippingTwoTone, text: "Giao hàng nhanh 2-4 giờ"       },
              ].map(({ Icon, text }) => (
                <div key={text} className="flex items-center gap-2 text-xs text-gray-500">
                  <Icon style={{ fontSize: 15 }} className="text-blue-500 flex-shrink-0" />
                  <span>{text}</span>
                </div>
              ))}
            </div>
